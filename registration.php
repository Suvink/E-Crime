...
if(isset($_POST['s'])){
    ...
    if($_SERVER["REQUEST_METHOD"]=="POST"){
       ...
       // $password=md5($u_pass);
	   $vkey=md5(time().$u_name);
	   session_start();
	   $_SESSION['vkey'] = $vkey;
	   
       $reg="insert into user (u_name,u_id,u_pass,u_addr,a_no,gen,mob,vkey) values('$u_name','$u_id','$u_pass','$u_addr','$a_no','$gen','$mob','$vkey')";
        mysqli_select_db($con,"crime");
		
        $res=mysqli_query($con,$reg)or die(mysqli_error($con));
        if(!$res)
        {
            ...
            else
    {
		...
		$message="<a href=http://localhost/E-Crime-Management-System-master/verify.php/?vkey='$vkey'>Register Account</a>";
		$headers  = 'From: pkarunarathne@yahoo.com' . "\r\n" .
		...
		$message = "You may now receive an email check that to register.";
		 ...
		 <div class="bg-agile">
			...
			<div class="login-form">	
				<form action="#" method="post">
					...
					<p style="color:#dfdfdf">ID Number</p><input type="text"  name="aadhar_number" minlength="10" maxlength="10"  id="aadh" onfocusout="f1()"/>
					...
					<div class="right-agileits">
						<p style="color:#dfdfdf">Mobile</p><input type="text"  name="mobile_number"  minlength="10" maxlength="10" id="mobno" onfocusout="f1()"/>
					</div>
